...
jobs:
  template-sync:
    ...
    steps:
      ...
      - uses: khulnasoft/action-template-sync@v2
        with:
...
jobs:
  template-sync:
    ...
    steps:
      ...
      - uses: khulnasoft-lab/action-workflow-run-wait@v1 # wait for workflow_run to be successful
      - uses: khulnasoft-lab/action-workflow-queue@v1 # avoid conflicts, by running this template one at a time
      - uses: khulnasoft/action-template-sync@v1
        with:
          github-token: ${{ secrets.GH_TOKEN }}
```
